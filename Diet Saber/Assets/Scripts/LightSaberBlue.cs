...
public class LightSaberBlue : MonoBehaviour
{
    ...
    private bool IsSccesstoSlice=false;
    void Start()
    {
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        {
            if(Vector3.Angle(other.gameObject.transform.up,_base.transform.position-_previousBasePosition)<45f)
            {
                _triggerEnterTipPosition = _tip.transform.position;
                _triggerEnterBasePosition = _base.transform.position;
                IsSccesstoSlice = true;
            }
            else 
            {
                IsSccesstoSlice = false;
            }
        }
        else return;
    }
